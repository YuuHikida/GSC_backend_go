...
依存するということの意味
DDDにおける「依存しない」とは、特定の層が他の層に強く結びつかないことを指します。具体的には、ドメイン層は他の層に依存せず、アプリケーション全体の中心的なビジネスルールを保つという意味です。

依存関係の正しい順番：
ドメイン層は他の層に依存しない（一番中心）。
アプリケーション層はドメイン層に依存する（ドメイン層のビジネスロジックを呼び出す）。
インフラ層はアプリケーション層とドメイン層に依存する（データベースとのやり取りや外部APIを扱う）。
インターフェース層はアプリケーション層に依存する（HTTPリクエストを受けてアプリケーション層のロジックを実行する）。
依存の具体例
OKな依存関係:
アプリケーション層がドメイン層のサービスを使ってビジネスルールを処理する。
インフラ層がドメイン層のモデルを使ってデータベースにアクセスする。
インターフェース層がアプリケーション層のサービスを使ってリクエスト処理を行う。
NGな依存関係:
ドメイン層がアプリケーション層やインフラ層を呼び出してはいけない。
ドメイン層はアプリケーションの核心部分で、他の層に影響を受けない純粋なビジネスロジックを持つべきだから。
インフラ層やインターフェース層が直接お互いを呼び出してはいけない。
それぞれの役割が明確に分けられているため、データベースロジック（インフラ層）とUIの処理（インターフェース層）を直接結びつけてはいけません。
依存関係の正しい流れ
ドメイン層は最も独立しており、他の層に影響されない。
アプリケーション層がドメイン層を呼び出してビジネスロジックを使う。
インフラ層がアプリケーション層やドメイン層のモデルを使ってデータベースや外部APIとのやり取りを行う。
インターフェース層がアプリケーション層のロジックを呼び出してリクエストに応じた処理を行う。
=======================

正しい依存関係
インターフェース層はアプリケーション層に依存し、ユーザーのリクエストを処理。
アプリケーション層は、インフラ層に依存して、データベースや外部APIのやり取りを指示。
インフラ層は、あくまでデータ操作を行い、アプリケーション層に結果を返すだけ。
まとめ
インフラ層はデータ操作を行い、他の層から指示を受けて処理する。
インターフェース層は外部からのリクエストを受け取り、ビジネスロジックに委譲する。
お互いを直接呼び出さないことで、各層の責務を分離し、柔軟で保守性の高いアーキテクチャを維持することができます。
...
アプリケーション層が呼び出すのは：
   ...
   ・インフラ層（データベースや外部API）
インターフェース層が呼び出すのは：
   ・アプリケーション層（ユースケースの処理やビジネスロジックを実行する）
インフラ層が呼び出すのは：
   ・データベースや外部システム（MongoDBや外部APIの実際の処理）